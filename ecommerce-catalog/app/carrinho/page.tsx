  ...
  const handleSavePedido = async () => {
    ...
    try {
      const pedido = await pedidoService.create({
        idCliente: user.id,
        idVendedor: items[0]?.product.idVendedor || "",
                  ...
                  <div key={item.product.id}>
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">{categorias.find((c) => c.id === item.product.idCategoria)?.nome || "Sem categoria"}</p>
                        <div className="flex gap-2 mt-1">
                          {item.product.desconto && parseFloat(item.product.desconto) > 0 && (
