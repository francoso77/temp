...
interface ProductCardProps {
  product: ProdutoInterface
}

export function ProductCard({ product }: ProductCardProps) {
  const [quantity, setQuantity] = useState(1)
  const { addItem } = useCart()
  const { isVendedor, vendedorId } = useAuth()
  ...
  useEffect(() => {
    const loadCategoria = async () => {
      if (product.idCategoria) {
        const cat = vendedorId
          ? await categoriaService.getById(product.idCategoria, vendedorId)
          : await categoriaService.getByIdFull(product.idCategoria)
        setCategoriaNome(cat?.nome || "Sem categoria")
      }
    }
    loadCategoria()
  }, [product.idCategoria, vendedorId])
        ...
        <div className="relative overflow-hidden rounded-md mb-4">
          ...
          {product.desconto && product.desconto > 0 && (
            <Badge variant="destructive" className="absolute top-2 left-2 bg-red-500/90 backdrop-blur-sm">
              -{(product.desconto * 1).toFixed(0).replace('.', ',')}% OFF
            </Badge>
          )}
