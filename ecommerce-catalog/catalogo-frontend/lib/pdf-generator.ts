...
// }
export const generateCatalogoPDF = async (produtos: ProdutoInterface[], vendedor: UserInterface, categorias: CategoriaInterface[]) => {
...
export const generateClientesPDF = async (clientes: ClienteInterface[], vendedor: UserInterface) => {
  const doc = new jsPDF()
...
export const generatePedidosPDF = async (pedidos: PedidoInterface[], vendedor: UserInterface) => {
  const doc = new jsPDF()
  ...
  const totalVendas = pedidos.reduce((sum, p) => sum + parseBRL(p.totalDescontado || 0), 0)
  ...
  // Labels de status
  const statusLabels: Record<PedidoInterface["status"], string> = {
    1: "Pendente",
    2: "Em Análise",
    3: "Aprovado",
    4: "Em Separação",
  ...
  const tableData = pedidos.map((pedido) => [
    ...
    // Corrigindo valor
    typeof pedido.totalDescontado === "number"
      ? pedido.totalDescontado.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })
      : typeof pedido.totalDescontado === "string"
        ? pedido.totalDescontado
        : "R$ 0,00",
    // Corrigindo status
    statusLabels[pedido.status] || "Desconhecido",
  ])
