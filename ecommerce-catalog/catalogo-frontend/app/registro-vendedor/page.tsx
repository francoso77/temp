...
export default function RegistroVendedorPage() {
  ...
  const { setMensagemState } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { confirmarSenha, ...dadosAEnviar } = formData
    const payload = {
      ...dadosAEnviar,
    ...
    try {
      const endpoint = `${URL_BACKEND}/auth/user`
      const response = await fetch(endpoint, {
        method: "POST",
        headers: {
          'Content-Type': 'application/json', // <--- ADICIONE ISTO!
        },
        body: JSON.stringify(payload), // <--- Use JSON.stringify!
      });
      ...
      if (rs.ok || response.ok) {
        ...
        setMensagemState({
          titulo: "Erro ...",
          exibir: true,
          mensagem: rs.message || 'Não foi possível completar o cadastro. Tente novamente.',
          tipo: "error",
          exibirBotao: true,
          cb: null
        })
