...
export default function ProductDetailPage() {
  ...
  const [product, setProduct] = useState<ProdutoInterface | null>(null)
  const [relatedProducts, setRelatedProducts] = useState<ProdutoInterface[]>([])
  ...
  const loadProduct = async (id: string) => {
    try {
      const allProducts = await produtoService.getAll()
      ...
      const nome = await categoriaService.getByIdFull(foundProduct.idCategoria as string).then(cat => cat.nome)
  ...
  const formatPercentage = (value: number) => {
    const num = (value * 1).toFixed(0).replace('.', ',')
    return num
  }

  const formatPrice = (price: number) => {
    const num = (price * 1).toFixed(2).replace('.', ',')
    return parseBRL(num).toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) //num.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })
  }
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            ...
            <div className="relative overflow-hidden rounded-lg bg-muted">
              ...
              {product.desconto && product.desconto > 0 && (
                <Badge variant="destructive" className="absolute top-16 right-4">
                  -{formatPercentage(product.desconto)}% OFF
                </Badge>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            {/* Price */}
            <Condicional condicao={isVendedor}>
                ...
                <div className="flex items-baseline space-x-2">
                  ...
                  {product.desconto && product.desconto > 0 && (
                    <>
                      ...
                      <Badge variant="destructive">-{formatPercentage(product.desconto)}%</Badge>
                    </>
                  )}
                  {product.promocao && (
