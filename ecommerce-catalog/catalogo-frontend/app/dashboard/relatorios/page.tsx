...
import { produtoService, clienteService, pedidoService, vendedorService, categoriaService } from "@/services/api"
...
export default function RelatoriosPage() {
  ...
  const handleGenerateCatalogo = async () => {
    try {
      setLoading("catalogo")
      const produtos = await produtoService.getAll(user!.idUsuario)
      const vendedor = await vendedorService.getProfile()
      const categorias = await categoriaService.getAll(user!.idUsuario)
  ...
  const handleGenerateClientes = async () => {
    try {
      setLoading("clientes")
      const clientes = await clienteService.getAll(user!.idUsuario)
      const vendedor = await vendedorService.getProfile()
  ...
  const handleGeneratePedidos = async () => {
    try {
      setLoading("pedidos")
      const pedidos = await pedidoService.getAll(user!.idUsuario)
