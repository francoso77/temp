...
export default function EditProductPage() {
  ...
  const formatNumberToBRL = (valor: number, tipo: 'preco' | 'desconto'): string => {
    if (tipo === 'preco') {
      ...
      return formatado.replace(/\s/g, ''); // Remove espaços em branco
  ...
  const loadProduct = async (id: string) => {
    try {
      ...
      setFormData({
        ...
        preco: formatNumberToBRL(foundProduct.preco, 'preco'),
        idCategoria: foundProduct.idCategoria,
        imagem: foundProduct.imagem || "",
        previewUrl: foundProduct.imagem || "",
        ativo: foundProduct.ativo,
        caracteristicas: foundProduct.caracteristicas || "",
        desconto: foundProduct.desconto ? formatNumberToBRL(foundProduct.desconto, 'desconto') : '0',
  ...
  const extrairNomeArquivo = (url: string): string => {
    if (!url) return "";
    try {
      const limpa = url.split("?")[0]; // remove query string se existir
      return limpa.replace(/\/+$/, "").split("/").pop() || "";
    } catch {
      return "";
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await produtoService.update({
        ...
        imagem: extrairNomeArquivo(formData.imagem) || "",
