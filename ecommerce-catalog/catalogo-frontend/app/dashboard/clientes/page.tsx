...
export default function ClientesPage() {
  ...
  const loadClientes = async () => {
    try {
      setLoading(true)
      const data = await clienteService.getAll(user!.idUsuario)
      setClientes(data)
  ...
  // const handleDeleteCliente = async (cliente: ClienteInterface) => {
  //   try {
  //     await clienteService.delete(cliente.id, user!.token)
  ...
  const handleDelete = async (id: string) => {
    ...
    const deleteCliente = clientes.find((c) => c.id === id)

    setMensagemState({
      titulo: 'Excluir Cliente ...',
      exibir: true,
      mensagem: "Tem certeza que deseja excluir o cliente " + deleteCliente?.nome + "? Esta ação não pode ser desfeita.",
      tipo: 'error',
      exibirBotao: "SN",
      cb: async (resposta: boolean) => {
        if (resposta) {
          try {
            await clienteService.delete(id, user!.token)
            await loadClientes()
          } catch (error) {
            console.error("Erro ao excluir cliente:", error)
            setMensagemState({
        ...
        <main className="flex-1 p-6">
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="max-w-sm  w-full md:max-w-none">
                <Card>
            ...
            {/* Filters */}
            <div className="md:w-full sm:w-full w-95">
              <Card className="mb-4">
                <CardContent className="p-4">
