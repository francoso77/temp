...
import { pedidoService, categoriaService } from "@/services/api"
...
export default function CartPage() {
  ...
  const handleSavePedido = async () => {
    ...
    try {
      ...
      //   idVendedor: items[0]?.produto.idVendedor || "",
      ...
      router.push(`/pedido-confirmado/${1}`)
    } catch (error) {
                  ...
                  <div key={item.produto.id}>
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">{categorias.find((c) => c.id === item.produto.idCategoria)?.nome || "Sem categoria"}</p>
                        <div className="flex gap-2 mt-1">
                          {item.produto.desconto && item.produto.desconto > 0 && (
                            <Badge variant="destructive" className="text-xs">
                              -{item.produto.desconto} OFF
                            </Badge>
                          )}
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateQuantity(item.produto.id, Math.max(1, Number.parseInt(e.target.value) || 1))
                          }
