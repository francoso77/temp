            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, data_source_1.AppDataSource.getRepository(entidade).find({
                                where: where,
                                select: select,
                                relations: relations,
                                order: order,
                    ...
                    case 1:
                        rs = _d.sent();
                        // 🔥 Ajustar imagens (somente para entidade Produto)
                        if (entidade === 'Produto') {
                            baseUrl_1 = process.env.BASE_URL || 'http://localhost:4000';
                            rs.forEach(function (item) {
                                if (item.imagem) {
                                    item.imagem = "".concat(baseUrl_1, "/uploads/produtos/").concat(item.imagem);
                                }
                            });
                        }
                        return [2 /*return*/, {
