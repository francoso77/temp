...
import { PedidoBackInterface, SomatorioPedidoInterface } from './PedidoDublagem';
...
export default function DetalhePedido({ rsMaster, setRsMaster, masterLocalState, setRsSomatorio }: PropsInterface) {
  ...
  const onEditar = (rs: DetalhePedidoDublagemInterface, indice: number) => {
  ...
  const onExcluir = (rs: DetalhePedidoDublagemInterface) => {
    if (rs.statusItem === StatusType.aberto) {

      let tmpDetalhe: Array<DetalhePedidoDublagemInterface> = []
  ...
  const btConfirmaInclusao = async () => {
    ...
    if (validarDados() && podeIncluirDetalhe() && estrutura) {
      let tmpDetalhe: Array<DetalhePedidoDublagemInterface> = [...rsMaster.detalhePedidos]
      tmpDetalhe.push({
  ...
  const AtualizaSomatorio = (rs: Array<DetalhePedidoDublagemInterface>) => {
          ...
          <DataTable
            ...
            acoes={masterLocalState.action === actionTypes.excluindo ? [] :
              [
                {
                  icone: "edit",
                  onAcionador: (rs: DetalhePedidoDublagemInterface, indice: number) =>
                    onEditar(rs, indice),
                  toolTip: "Editar",
                },
                {
                  icone: "delete",
                  onAcionador: (rs: DetalhePedidoDublagemInterface) =>
                    onExcluir(rs),
                  toolTip: "Excluir",
                },
              ]}
