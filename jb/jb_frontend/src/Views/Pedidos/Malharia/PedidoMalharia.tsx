...
import DetalhePedidoTinturaria from './DetalhePedidoMalharia';
...
export default function PedidoMalharia() {
  ...
  const [pesquisa, setPesquisa] = useState<PesquisaInterface>({ itemPesquisa: '', idCliente: 0, idFornecedor: 0, status: StatusType.todos })
  ...
  const pesquisarID = async (id: string | number): Promise<PedidoMalhariaInterface | null> => {
    const rs = await clsCrud
      .pesquisar({
        entidade: "PedidoMalharia",
        relations: [
          "cliente",
          "fornecedor",
          "detalhePedidoMalharias",
          "detalhePedidoMalharias.produto",
        ],
        criterio: {
          idPedido: id,
        },
  ...
  const validarDados = (): boolean => {
    ...
    retorno = validaCampo.naoVazio('idPessoa_fornecedor', pedido, erros, retorno, 'Informe a tinturaria')

    setErros(erros)
    return retorno
  }

  const AtualizaSomatorio = (rs: PedidoMalhariaInterface) => {

    let totalQtd: number = 0
    let total: number = 0

    if (rs.detalhePedidoMalharias) {
      rs.detalhePedidoMalharias.forEach((detalhe) => {
  ...
  const criarNotification = async (cli: string, ped: string, tin: string, pedido: any) => {
      ...
      if (localState.action === actionTypes.incluindo || localState.action === actionTypes.editando) {
        clsCrud.incluir({
          ...
          .then((rs) => {
            ...
            const tin = rsFornecedor.find(c => c.idPessoa === pedido.idPessoa_fornecedor)?.nome || 'TINTURARIA NÃO ENCONTRADA';

            if (rs.ok) {
              setLocalState({ action: actionTypes.pesquisando })
              criarNotification(cli, String(rs.dados.idPedido), tin, rs.dados)
            } else {
              setMensagemState({
                titulo: 'Erro...',
                exibir: true,
  ...
  const btPesquisar = () => {
    ...
    // 👔 Filtro por vendedor
    if (pesquisa.idFornecedor && pesquisa.idFornecedor > 0) {
      filtrados = filtrados.filter(
        (pedido) => pedido.idPessoa_fornecedor === pesquisa.idFornecedor
      );
  ...
  const btFechar = () => {
    setLayoutState({
      ...layoutState,
      titulo: '',
      tituloAnterior: 'Cadastro de Pedidos de Tinturaria',
      pathTitulo: '/',
      pathTituloAnterior: '/PedidoMalharia',
    })
    irPara('/')
  }
          ...
          <Condicional condicao={localState.action === 'pesquisando'}>
            ...
            <Grid item xs={12} md={4}>
              <ComboBox
                ...
                campoID="idPessoa"
                dados={pesquisa}
                mensagemPadraoCampoEmBranco="Escolha a tinturaria"
                field="idFornecedor"
                label="Tinturaria"
                erros={erros}
                setState={setPesquisa}
